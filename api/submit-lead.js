...
const ZIP_DEFAULT = '54082'; // Houlton, WI
const ADDRESS_DEFAULT = 'Houlton, WI';
const SITE_SOURCE_TAG = 'Site: medicalofficecleaninghoulton.com';
...
function buildWizardPayload(body) {
  ...
  const rawNotes = clean(input.notes);
  const notes = rawNotes ? `${SITE_SOURCE_TAG} | ${rawNotes}` : SITE_SOURCE_TAG;
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // while validation failures come back as 404 with an HTTP 400.
